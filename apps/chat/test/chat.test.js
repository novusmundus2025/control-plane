...
test("renders local-first Projects without a separate Computer surface", () => {
  ...
  assert.match(projects, /id="harness-pair"[^>]*hidden disabled>Connect</);
  assert.match(projects, /id="project-runner-setup" hidden/);
  assert.match(projects, /id="runner-progress" data-stage="install"/);
  assert.match(projects, />Install<\/small>[\s\S]*>Connect<\/small>[\s\S]*>Ready<\/small>/);
  assert.match(projects, /Install local runner/);
  assert.match(projects, /Connect browser/);
  assert.match(projects, /id="harness-installed"[^>]*hidden>I installed it/);
  assert.match(projects, /Creates a one-time code for this runner\. GitHub access is separate/);
  assert.match(html, /Install the runner now\. Secure browser pairing will be available when account connection is enabled/);
  assert.match(html, /runnerInstallationAcknowledged = localStorage\.getItem\("mundusx\.chat\.runnerInstalled\.v1"\) === "true"/);
  assert.match(html, /harnessPairEl\.disabled = !pairingAvailable/);
  assert.match(html, /runnerPairingPollTimer = window\.setTimeout/);
  assert.match(html, /Waiting for the runner to use the pairing code/);

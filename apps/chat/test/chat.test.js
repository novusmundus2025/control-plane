...
test("renders local-first Projects without a separate Computer surface", () => {
  ...
  assert.match(projects, /id="harness-pair"[^>]*hidden>Connect runner/);
  assert.match(projects, /Set up local runner/);
  assert.match(projects, /id="project-runner-setup" hidden/);
  assert.match(projects, /Install the local runner/);
  assert.match(projects, /Connect this browser/);
  assert.match(projects, /Pairing authorizes local project actions; it is separate from GitHub access/);
  assert.match(html, /Install the runner now\. Secure browser pairing will be available when account connection is enabled/);
  assert.match(html, /harnessPairEl\.hidden = !pairingAvailable/);

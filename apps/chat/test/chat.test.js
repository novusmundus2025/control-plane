...
test("renders local-first Projects without a separate Computer surface", () => {
  ...
  assert.match(projects, /mundusx\/releases\/releases\/download\/cli-windows-v0\.2\.04\/MundusX-Setup\.exe/);
  assert.match(html, /const minimumLocalAgentVersion = "0\.1\.87"/);
  assert.match(html, /const latestLocalAgentVersion = "0\.2\.04"/);
  assert.match(html, /projectReadinessEl\.dataset\.state = updateAvailable \? "update"/);
  assert.match(html, /projectReadinessTitleEl\.textContent = updateRequired \? "Update required" : updateAvailable \? "Update available"/);
...
test("every non-empty normal Chat request uses the direct stream", () => {  assert.equal(canLiveStreamChatTurn({ message: "Explain distributed systems.", toolMode: false }), true);
...
test("native Hermes capability discovery is reused across project model turns", async () => {
  ...
  const config = configFromEnv({
    ...
    MUNDUSX_AGENT_MODEL_BASE_URLS: "https://control.mundusx.ai",
...
test("normal Chat streams direct even for metadata-like prompts", async () => {  const events = [];
  const encoder = new TextEncoder();
  const response = {
    writableEnded: false,

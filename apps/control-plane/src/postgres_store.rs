...
impl PostgresStore {
    ...
    pub fn restore_state(&self) -> Result<ControlPlaneState, String> {
        ...
        let job_events: Vec<JobEventRecord> = query_json_array(&mut client, JOB_EVENTS_RESTORE_SQL)?;
        let credits_ledger: Vec<CreditsLedgerRecord> =
            query_json_array(&mut client, CREDITS_RESTORE_SQL)?;
...
select coalesce(jsonb_agg(to_jsonb(t) order by t.source_event_id asc nulls last, t.id asc)::text, '[]')
from (
  select *
